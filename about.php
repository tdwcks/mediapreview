<!DOCTYPE html>
<html>
<head>
<link href="style.css" rel="stylesheet" type="text/css">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>MediaPreview &#8594; About</title>
</head>
<body>
	<?php include 'includes/header.php'; ?>
	<article class="about">
		<section class="hero">
			<div class="grid-container">
				<div class="row">
					<div class="col-6">
						<h2>We produce video programmes and products for commercial, public sector and community clients.</h2>
					</div> <!-- col-6 -->
				</div> <!-- row -->
			</div> <!-- grid-container -->
		</section> <!-- hero -->
		<section class="longform">
			<div class="grid-container">
				<div class="row">
					<div class="col-4">
						<p>Mediadpreview's Andy Wicks and his associates have been making videos and screen media products for commercial, public sector and community clients for the past 30 years.</p>
					</div> <!-- col-4 -->
					<div class="col-6">
						<p>We drill down find out what the client really needs and make sure it gets delivered to the highest standard, on time and on budget.</p>
						<p>On the production side, using state of the art 4k DSLR technology, we work unobtrusively, striking the balance between putting interviewees at ease and shooting useable material that shows everyone in the best light - literally and metaphorically! We have extensive experience of shooting and editing green screen.</p>
						<p>Post - production wise we specialise in Adobe Premiere and After Effects editing. Working creatively and speedily - finding the narrative, managing large amounts of material to assemble films that flow seamlessly with a light touch, bring out the best in the participants and realise their original purpose.</p>
					</div> <!-- col-6 -->
				</div> <!-- row -->
			</div> <!-- grid-container -->
		</section> <!-- longform -->
		<section class="testimonials">
			<div class="swiper-container">
	        <div class="swiper-wrapper">
	            <div class="swiper-slide">
	            	<div class="grid-container">
	            		<div class="col-8">
	            			<h2>"Mediapreview and Longley Farm have worked together for nearly twenty years now. They can put together all that is needed from idea to edit; from script to soundtrack. They are totally reliable and you can be sure of a good result."</h2>
	            			<h5>Jim Dickinson, Longley Farm</h5>
	            		</div>
	            	</div>
	            </div>
	            <div class="swiper-slide">
	            	<div class="grid-container">
	            		<div class="col-8">
	            			<h2>"Mediapreview are a highly professional company who marry first class technical skills to a ready understanding  of a brief, and a passion for getting behind a subject."</h2>
	            			<h5>Nancy Barrett, Creative Scene</h5>
	            		</div>
	            	</div>
	            </div>
	            <div class="swiper-slide">
	            	<div class="grid-container">
	            		<div class="col-8">
	            			<h2>Mediapreview keep leaving their chewing gum on the side of the table and it's really annoying.</h2>
	            			<h5>Tom Wicks, Miln</h5>
	            		</div>
	            	</div>
	            </div>
	        </div>
	        <!-- Add Pagination -->
	        <div class="swiper-pagination"></div>
	        <!-- Add Arrows -->
	        <div class="swiper-button-next"></div>
	        <div class="swiper-button-prev"></div>
	    </div>
		</section>
		<section class="longform-breakdown">
			<div class="grid-container">
				<div class="row">
					<div class="col-4">
						<p>Services love making things, collaborating, identifying interesting problems and figuring out what we can build to help solve them</p>
					</div> <!-- col-4 -->
					<div class="col-3">
						<h4>Video Production</h4>
						<p>Bringing together multiple disciplines to design and build websites that meet the needs of your organisation.</p>
						<ul>
							<li>Sick Videos</li>
							<li>Another thing</li>
							<li>Sick Videos </li>
						</ul>
					</div>
					<div class="col-3">
						<h4>Editing</h4>
						<p>Bringing together multiple disciplines to design and build websites that meet the needs of your organisation.</p>
						<ul>
							<li>Sick Videos</li>
							<li>Another thing</li>
							<li>Sick Videos </li>
						</ul>
					</div>
				</div> <!-- row -->
				<div class="row">
					<div class="col-4">
						<p></p>
					</div> <!-- col-4 -->
					<div class="col-3">
						<h4>Video Production</h4>
						<p>Bringing together multiple disciplines to design and build websites that meet the needs of your organisation.</p>
						<ul>
							<li>Sick Videos</li>
							<li>Another thing</li>
							<li>Sick Videos </li>
						</ul>
					</div>
					<div class="col-3">
						<h4>Editing</h4>
						<p>Bringing together multiple disciplines to design and build websites that meet the needs of your organisation.</p>
						<ul>
							<li>Sick Videos</li>
							<li>Another thing</li>
							<li>Sick Videos </li>
						</ul>
					</div>
				</div> <!-- row -->
			</div> <!-- grid-container -->
		</section> <!-- longform-breakdown -->
		<section class="image-gallery">
			<div class="grid-container">
				<div class="row">
					<div class="col-6">
						<div class="image">
							
						</div> <!-- image -->
					</div>  <!-- col-6 -->
					<div class="col-6">
						<div class="image">
							
						</div> <!-- image -->
					</div> <!-- col-6 -->
				</div> <!-- row -->
				<div class="row">
					<div class="col-6">
						<div class="image">
							
						</div> <!-- image -->
					</div>  <!-- col-6 -->
					<div class="col-6">
						<div class="image">
							
						</div> <!-- image -->
					</div> <!-- col-6 -->
				</div> <!-- row -->

			</div> <!-- grid-container -->
		</section> <!-- longform-breakdown -->
	</article> <!-- about -->
	<?php include 'includes/footer.php'; ?>
</body>

</html>